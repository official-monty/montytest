...
import requests
from montytest.schemas import runs_schema, short_worker_name
...
def login(request):
    ...
    if request.method == "POST":
        ...
        if "Account pending for user:" in message:
            message += (
                " . If you recently registered to montytest, "
...
Thank you for contributing to the development of Monty. Unfortunately, it seems your montytest worker {worker_name} has some issue(s). More specifically the following has been reported:

{message}

You may possibly find more information about this in our event log at {host_url}/actions

Feel free to reply to this email if you require any help, or else contact the #montytest channel on the Monty Discord server: https://discord.gg/VvAyXtHDyJ

Enjoy your day,

{blocker_name} (Montytest approver)
...
def signup(request):
    ...
    path = os.path.expanduser("~/montytest.captcha.secret")
...
def validate_form(request):
    ...
    if stop_rule == "sprt":
        ...
        data["sprt"] = montytest.stats.stat_util.SPRT(
            alpha=0.05,
            beta=0.05,
