...
<%!
  ...
  import montytest.stats.LLRcalc
  import math
  ...
  z975 = montytest.stats.stat_util.Phi_inv(0.975)
...
<%
  ...
  results3_ = montytest.stats.LLRcalc.regularize(results3)
  draw_ratio = results3_[1] / float(sum(results3_))
  N3, pdf3 = montytest.stats.LLRcalc.results_to_pdf(results3)
  games3 = N3
  avg3, var3, skewness3, exkurt3 = montytest.stats.LLRcalc.stats_ex(pdf3)
  stdev3 = var3 ** 0.5
  games = games3
  sigma = stdev3
  ...
  if has_pentanomial:
      ...
      results5_ = montytest.stats.LLRcalc.regularize(results5)
      pentanomial_draw_ratio = results5_[2] / float(sum(results5_))
      N5, pdf5 = montytest.stats.LLRcalc.results_to_pdf(results5)
      games5 = 2 * N5
      avg5, var5, skewness5, exkurt5 = montytest.stats.LLRcalc.stats_ex(pdf5)
      ...
      RMS_bias_elo = montytest.stats.stat_util.elo(0.5 + RMS_bias)

  drawelo = montytest.stats.stat_util.draw_elo_calc(results3_)
  if has_sprt:
      ...
      if elo_model == "BayesElo":
          ...
          elo0_, elo1_ = [
              montytest.stats.stat_util.bayeselo_to_elo(belo_, drawelo)
      ...
      if elo_model_ == "logistic":
          ...
          score03, score13 = score0, score1 = [
              montytest.stats.stat_util.L(lelo_) for lelo_ in (lelo0, lelo1)
          ]
          nelo0, nelo1 = [
          ...
          lelo0, lelo1 = [
              montytest.stats.stat_util.elo(score_) for score_ in (score0, score1)
          ]
          lelo03, lelo13 = [
              montytest.stats.stat_util.elo(score_) for score_ in (score03, score13)
          ]

      if belo0 is None:
          belo0, belo1 = [
              montytest.stats.stat_util.elo_to_bayeselo(lelo_, draw_ratio)[0]
              for lelo_ in (lelo03, lelo13)
          ]
      LLRjumps3 = list_to_string(
          [i[0] for i in montytest.stats.LLRcalc.LLRjumps(pdf3, score0, score1)]
      )
      sp = montytest.stats.sprt.sprt(alpha=alpha, beta=beta, elo0=lelo0, elo1=lelo1)
      sp.set_state(results3_)
      a3 = sp.analytics()
      LLR3_l = a3["a"]
      LLR3_u = a3["b"]
      if elo_model_ == "logistic":
          LLR3 = montytest.stats.LLRcalc.LLR_logistic(lelo03, lelo13, results3_)
      else:  # normalized
          LLR3 = montytest.stats.LLRcalc.LLR_normalized(nelo03, nelo13, results3_)
      ...
      LOS3 = a3["LOS"]
      # auxilliary
      LLR3_exact = N3 * montytest.stats.LLRcalc.LLR(pdf3, score03, score13)
      LLR3_alt = N3 * montytest.stats.LLRcalc.LLR_alt(pdf3, score03, score13)
      LLR3_alt2 = N3 * montytest.stats.LLRcalc.LLR_alt2(pdf3, score03, score13)
      LLR3_normalized = montytest.stats.LLRcalc.LLR_normalized(nelo03, nelo13, results3_)
      LLR3_normalized_alt = montytest.stats.LLRcalc.LLR_normalized_alt(
          nelo03, nelo13, results3_
      )
      LLR3_be = montytest.stats.stat_util.LLRlegacy(belo0, belo1, results3_)
      if has_pentanomial:
          LLRjumps5 = list_to_string(
              [i[0] for i in montytest.stats.LLRcalc.LLRjumps(pdf5, score0, score1)]
          )
          sp = montytest.stats.sprt.sprt(alpha=alpha, beta=beta, elo0=lelo0, elo1=lelo1)
          ...
          LLR5_u = a5["b"]
          if elo_model_ == "logistic":
              LLR5 = montytest.stats.LLRcalc.LLR_logistic(lelo0, lelo1, results5_)
          else:  # normalized
              LLR5 = montytest.stats.LLRcalc.LLR_normalized(nelo0, nelo1, results5_)
          ...
          LOS5 = a5["LOS"]
          # auxilliary
          LLR5_exact = N5 * montytest.stats.LLRcalc.LLR(pdf5, score0, score1)
          LLR5_alt = N5 * montytest.stats.LLRcalc.LLR_alt(pdf5, score0, score1)
          LLR5_alt2 = N5 * montytest.stats.LLRcalc.LLR_alt2(pdf5, score0, score1)
          LLR5_normalized = montytest.stats.LLRcalc.LLR_normalized(nelo0, nelo1, results5_)
          LLR5_normalized_alt = montytest.stats.LLRcalc.LLR_normalized_alt(
              nelo0, nelo1, results5_
          )
  else:  # assume fixed length test
      elo3, elo95_3, LOS3 = montytest.stats.stat_util.get_elo(results3_)
      elo3_l = elo3 - elo95_3
      elo3_u = elo3 + elo95_3
      if has_pentanomial:
          elo5, elo95_5, LOS5 = montytest.stats.stat_util.get_elo(results5_)
          elo5_l = elo5 - elo95_5
          elo5_u = elo5 + elo95_5
%>

<script>
  document.title = "Statistics - ${page_title} | Stockmonty Testing";
</script>
